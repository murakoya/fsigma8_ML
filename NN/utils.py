...
def filenames_and_labels(
    path_to_data, split_list, filename, labelfile, num_train=1500, num_val=100
):
    all_box_train = [
        path_to_data + "/dir{}/{}".format(int(split_list[i]), filename)
        for i in range(num_train)
    ]

    all_box_val = [
        path_to_data
        + "/dir{}/{}".format(int(split_list[i + num_train]), filename)
    ...
    all_box_test = [
        path_to_data
        + "/dir{}/{}".format(
            int(split_list[int(i + num_train + num_val)]), filename
        )
    ...
    all_label_train = [
        np.array(
            [
                np.loadtxt(
                    path_to_data
                    + "/dir{}/{}".format(int(split_list[i]), labelfile)
                )
            ]
        )
        for i in range(num_train)
        ...
        np.array(
            [
                np.loadtxt(
                    path_to_data
                    + "/dir{}/{}".format(
                        int(split_list[i + num_train]), labelfile
                    )
                )
            ]
        )
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.transform == None:
            box = torch.tensor(
                np.loadtxt(self.file_names[idx], unpack=True)[1],
                dtype=torch.float32,
            )
...
def run_training(
    ...
    for epoch in range(num_epochs):
        ...
        if test_loss < min_val:
            ...
            with open(
                output_dir + "/min_val_model_{}.p".format(start_epoch), "wb"
            ) as f:
                pickle.dump(model, f)
    ...
    print(
        "min_val_model is saved at epoch {}.".format(min_epoch + start_epoch)
    )
