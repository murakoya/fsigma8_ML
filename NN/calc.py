...
filename = params['data_io']['filename']
labelfile = params['data_io']['labelfile']
...
num_train = params['config_input']['num_train'] # number of data
for training
...
mean, std = calc_mean_std_of_output(
    path_to_data=input_dir, filename=labelfile, num_data=num_data
)
...
if load_model != None:
    if os.path.isfile(load_model) == True:
        ...
        train_loss_list, val_loss_list = np.load(
        ...
        summary(
            model,
            (int(batch_size), int(ch), int(img_size), int(img_size)),
...
if prediction == False:
    ...
    # images_test, labels_test = filenames_and_labels(input_dir, phase="test")
    # transform_test = MyTransformer(
    #    img_size=int(img_size), ch=int(ch), phase="test"
    # )
    # dataset_test = MyDataset(
    #    file_names=images_test, labels=labels_test, transform=transform_test
    # )
    # test_loader = torch.utils.data.DataLoader(
    #    dataset_test, batch_size=1, shuffle=False
    # )
    ...
    run_test(
        model=model,
        filename_list=images_train,  ## <- changed
